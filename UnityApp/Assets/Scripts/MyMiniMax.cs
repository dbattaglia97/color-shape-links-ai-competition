    ...
    public class MyMiniMax : AbstractThinker
    {
        ...
        private float Heuristic(Board board, PColor color)
        {
            ...
            float BuildFromAfar(float x,float y){
                ...
                if(y<winningStrike-1||y>board.cols-winningStrike) return val;

                for (int j=(int)y+1; j<(int)y+winningStrike-1;j++){
                ...
                for (int j = 0; j < board.cols; j++)
                {
                    ...
                    if (piece.HasValue)
                    {
                        ...
                        if (piece.Value.shape == color.Shape()){
                            h += 1.5f*HorizontalHeuristic(i);
                            h += 1.5f*VerticalHeuristic(j);
                            h += 1.5f*BuildFromAfar(i,j);
                        }
                        // Otherwise decrement the heuristic value using the
                        // same criteria
                        else{
